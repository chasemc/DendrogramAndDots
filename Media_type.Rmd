...
numiter<- if(length(colrs) < length(bigMatrix)){
  length(colrs)
}else{ length(bigMatrix) }

for (i in 1:numiter){
  
   
  
bigMatrix[,i][which(bigMatrix[,i]=="#000000"),]<-colrs[[i]]}
...
```{r,fig.height=175}
    ...
    colored_dots(bigMatrix, dend,
         rowLabels = names(bigMatrix),horiz=T,sort_by_labels_order = FALSE,cex.rowLabels = .7) 
