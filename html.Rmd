...
groupedList<-split(small,factor(small$`Sample type 1`))
...
for(x in 1:length(bigList)){
    bigList[[x]]$`Sample type 1`[!is.na(bigList[[x]]$`Sample type 1`)]<-"#000000"
    bigList[[x]]$`Sample type 1`[is.na(bigList[[x]]$`Sample type 1`)]<-"#00000000"
}
...
colored_dots(cbind(bigList$algae$`Sample type 1`), dend,
