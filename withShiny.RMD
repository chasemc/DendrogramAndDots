...
shinyApp(
  ...
  ui = fluidPage(
        numericInput("hclustHeight", label = h5("Expand Tree"),value = 5000,step=50,min=100),
      mainPanel("Hierarchical Clustering",plotOutput("netheir"))        
  ),
  server = function(input, output) {
   plotHeight <- reactive({
    return(as.numeric(input$hclustHeight))
  })
  output$netheir <- renderPlot(execOnResize = FALSE,{
    par(mar = c(8,8,8,25))
